...
(define-library (labeled-graph)
  ...
  (begin
    ...
    (define (add-edge! graph from to label)
      (define rows (storage graph))
      (when (if (directed? graph) ; only count new edge if there wasn't one before
                (eq?  (vector-ref (cdr (vector-ref rows from)) to) 'no-label)
                (and (eq?  (vector-ref (cdr (vector-ref rows from)) to) 'no-label)
                     (eq?  (vector-ref (cdr (vector-ref rows to)) from) 'no-label)))
    ...
    (define (delete-edge! graph from to)
      (define rows (storage graph))
      (unless (if (directed? graph) ; only decrease counter if there was an edge before
                  (eq?  (vector-ref (cdr (vector-ref rows from)) to) 'no-label)
                  (and (eq?  (vector-ref (cdr (vector-ref rows from)) to) 'no-label)
                       (eq?  (vector-ref (cdr (vector-ref rows to)) from) 'no-label)))
        (nr-of-edges! graph (- (nr-of-edges graph) 1)))
